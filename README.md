...
v0.9.1 Edge semantic contract  — trust_map 元数据补全，生产静默退化修复
v0.9.2 AttackGraphEngine       — 统一六层管线 API + CLI analyze + 实战验证
v0.9.3 ILP MCS                  — PuLP 整数规划最小割集，精确求解最优断边
v0.10  Live RBAC Adapter          — 实时 K8s RBAC 查询构建 TrustEdge，替代硬编码拓扑
```
...
| 版本 | 日期 | 关键变更 |
|------|------|----------|
| **v0.10.0** | 2026-06-14 | 🔌 **Live RBAC Adapter** — 实时 K8s RBAC 查询构建 TrustEdge，`build_live_topology()` 替代硬编码拓扑 |
| **v0.9.3** | 2026-06-14 | 🧮 **ILP MCS** — PuLP+CBC 整数规划最小割集，精确求解最优断边（ILP vs Greedy 双轨+回退） |
| **v0.9.2** | 2026-06-14 | 🚀 **AttackGraphEngine** — 统一六层管线 API + CLI `analyze` 命令 + 13 集成契约测试 + minikube 实战验证 |
| **v0.9.1** | 2026-06-14 | 🔧 **Edge Semantic Contract Fix** — trust_map 元数据补全 + identity 回退读取，零回归 |
...
| `runtime/minimal_cut.py` | **MCS 最小割集** — ILP (PuLP+CBC) + Greedy 双轨 hitting-set，精确求解最优断边 | v0.7.0/0.9.3 |
| `runtime/classifier.py` | **AttackLabel 语义投影** — 5 tactic 分类 | v0.8.0 |
| `runtime/engine.py` | **AttackGraphEngine** — 六层管线统一 API，一行构建 | v0.9.2 |
...
│   │   ├── engine.py          # AttackGraphEngine 统一管线 API
...
│   │   ├── trust_map.py       # 信任拓扑映射
│   │   ├── rbac_adapter.py    # 🆕 Live RBAC Adapter (v0.10)
...
└── tests/
    ...
    ├── test_recon_to_runtime_contract.py  # 13 条 (trust_map→runtime 契约)
    ...
    └── test_rbac_adapter.py   # 🆕 46 条 (Live RBAC Adapter)
```
...
| `analyze` | **攻击图全管线分析** — G→S→T→Δ→MCS→Label 六层 | **v0.9.2** | `--full-pipeline`, `--quick`, `--json`, `--entry`, `--target` |
| `export` | 多格式导出 | v0.2.0 | — |
...
```
--full-pipeline          运行全部六层管线（G→S→T→Δ→MCS→Label）
--quick                  快速模式（仅 G+S+T）
--json                   输出 JSON 格式
--entry TEXT             入口身份（默认: low-privilege-sa）
--target TEXT            目标资产（默认: cluster-admin）
--compromise-threshold TEXT  妥协阈值: SAFE / PARTIAL / COMPROMISED
```
...
| 类别 | 数量 |
|------|------|
| **🔷 攻击图状态机** (`runtime/`) | **10 模块** (models + engine + 5 核心 + classifier + invariants) |
...
**总测试用例: 449**（449 passed, 3 skipped, 0 failed）— pytest, ~16ms/test
...
| `tests/test_minimal_cut.py` | 20 | MCS: ILP (PuLP) + greedy/exact hitting-set, 割集验证, 边数量边界 |
...
| `tests/test_recon_to_runtime_contract.py` | 13 | trust_map→runtime 端到端契约验证 |
...
| `tests/test_rbac_adapter.py` | 46 | Live RBAC Adapter 推理、边生成、集成测试 |
